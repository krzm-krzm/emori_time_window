...
def capacity(route):
    ...
    for i in range(len(route)):
        for j in range(len(route[i])):
            ...
            if q[i] > Q_max:
                capacity_over += q[i]-Setting_Info[3]
...
def capacity_route_k(route_k):
    ...
    for i in range(len(route_k)):
        ...
        if q > Q_max:
            capacity_over += q-Setting_Info[3]
    return capacity_over
    ...
    if not len(Route_k) == 0:
        for i in range(len(Route_k)):
            if i == 0:
                ...
                D[Route_k[i]] = B[Route_k[i]] + abs(noriori[Route_k[i]])
...
def swap(route, requestnode, tabu_list, best_neighbour):
    ...
    for i in range(vhiecle):
        vhiecle_list[i] = penalty_sum_route_k(route[i], requestnode)
...
def main(LOOP):
    ...
    print(keisu)
    # print(tabu_list)
    #np.savetxt('/home/kurozumi/デスクトップ/data/A_greedy_hs'+FILENAME+'.csv', data, delimiter=",")
...
if __name__ == '__main__':
    ...
    d = 3  # 乗り降りの時間
    Q_max = Setting_Info[3]  # 車両の最大容量 global変数 capacity関数で使用
    T_max = Setting_Info[2]  # 一台当たりの最大移動時間
    L_max = Setting_Info[4]  # 一人あたりの最大移動時間
